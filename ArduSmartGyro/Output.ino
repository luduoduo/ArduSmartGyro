...
void output_angles()
{
  ...
  else if (output_format == OUTPUT__FORMAT_SHORT)
  {
    Serial.print("@");  //mark

    int yaw_i = shortForFloat((TO_DEG(yaw)));
    int pitch_i = shortForFloat(TO_DEG(pitch));
    int roll_i = shortForFloat(TO_DEG(roll));

    printHolder(yaw_i);
    Serial.print(abs(yaw_i));
    printHolder(pitch_i);
    Serial.print(abs(pitch_i));
    printHolder(roll_i);
    Serial.print(abs(roll_i));
    Serial.println();
  }
}
